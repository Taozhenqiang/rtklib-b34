...
const prcopt_t prcopt_default={
    ...
    1,
    1,
    0,/* armaxiter,sysisb,estion,ionoise,esttrop,dynamics,tidecorr */
    1,
    0,
    0,
    0,
...
/* satellite system to signal frequency index ----------------------------------------
...
static int code2freq_BDS(uint8_t code,double *freq) {
    ...
    case '2':
        *freq=FREQ1_CMP;
        return 0;/* B1I */
    case '7':
        if ((strchr(string,obs[1])-string) >=3) {
            *freq=FREQ2_CMP;
            return 5;/* B2b */
        } else {
            *freq=FREQ2_CMP;
...
/* add satellite antenna parameter -----------------------------------------------------*/
static void addspcv(const spcv_t *pcv, spcvs_t *pcvs) {
    spcv_t *pcvs_pcv;

    if (pcvs->nmax<=pcvs->n) {
        pcvs->nmax+=256;
        if (!(pcvs_pcv=(spcv_t *) realloc(pcvs->pcv,sizeof(spcv_t)*pcvs->nmax))) {
...
/* add receiver antenna parameter -----------------------------------------------------*/
static void addrpcv(const rpcv_t *pcv, rpcvs_t *pcvr) {
    rpcv_t *pcvr_pcv;

    if (pcvr->nmax<=pcvr->n) {
        pcvr->nmax+=256;
        if (!(pcvr_pcv=(rpcv_t *) realloc(pcvr->pcv,sizeof(rpcv_t)*pcvr->nmax))) {
            trace(1,"addpcv: memory allocation error\n");
            free(pcvr->pcv);
            pcvr->pcv=NULL;
            pcvr->n=pcvr->nmax=0;
...
static int readngspcv(const char *file,spcvs_t *pcvs) {
    FILE *fp;
    static const spcv_t spcv0={0};
    spcv_t spcv;
    double neu[3];
    int n=0;
    char buff[256];
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (++n==1) {
            ...
            decodef(buff,10,spcv.var[0]);
        else if (n==4)
            decodef(buff,9,spcv.var[0]+10);
        else if (n==5) {
            if (decodef(buff,3,neu)<3)
                continue;
            spcv.off[1][0]=neu[1];
            ...
            decodef(buff,10,spcv.var[1]);
        else if (n==7) {
            decodef(buff,9,spcv.var[1]+10);
            addspcv(&spcv,pcvs);
        }
    }
    fclose(fp);
...
/* read antex file ----------------------------------------------------------*/
static int readantex(const char *file, spcvs_t *pcvs, rpcvs_t *pcvr) {
    FILE *fp;
    static const spcv_t spcv0={0};
    static const rpcv_t rpcv0={0};
    spcv_t spcv;
    rpcv_t rpcv;
    double neu[3];
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (strstr(buff+60,"TYPE / SERIAL NO")) {
            ...
            if (1==flag1) {
                spcv.dazi=str2num(buff,2,6); continue;
            }
            else if (1==flag2) {
            ...
            if (1==flag1) {
                ...
                spcv.dzen=str2num(buff,14,6);
                continue;
            }
            else if (1==flag2) {
                rpcv.zen1=str2num(buff,2,6); 
                rpcv.zen2=str2num(buff,8,6);
                rpcv.dzen=str2num(buff,14,6);
                continue;
            }
		} 
        else if (strstr(buff+60,"VALID FROM")) {
            if (1==flag1) {
                if (!str2time(buff,0,43,&spcv.ts)) continue;
            }
            else if (1==flag2) {
                if (!str2time(buff,0,43,&rpcv.ts)) continue;
            }
        } 
        else if (strstr(buff+60,"VALID UNTIL")) {
            if (1==flag1) {
                if (!str2time(buff,0,43,&spcv.te)) continue;
            }
            else if (1==flag2) {
                if (!str2time(buff,0,43,&rpcv.te)) continue;
            }
        } 
            ...
            if (freq<1) continue;
            if (1==flag1) {
                spcv.off[freq-1][0]=neu[0];     /* x */
                spcv.off[freq-1][1]=neu[1];     /* y */
        ...
        /* satellite and receiver PCV considers changes with azimuth angle*/
            ...
            if (1==flag1) {
                num=(int)((spcv.zen2-spcv.zen1)/spcv.dzen+1);
                if (0==spcv.dazi) {
                    if ((i=decodef(buff+8,num,spcv.var[freq-1]))<=0) 
                        continue;
                }
                else if (spcv.dazi>0) {
                    id=(int)(360.0/spcv.dazi)+1;
                    for (i=0;i<id;i++) {
                        fgets(buff,sizeof(buff),fp);
                        if ((j=decodef(buff+8,num,&spcv.var[freq-1][i*num]))<=0)
                            continue;
				    }
                }
            }
 ...
 *-----------------------------------------------------------------------------*/
extern int readpcv(const char *file,spcvs_t *pcvs,rpcvs_t *pcvr) {
    spcv_t *spcv;
    char *ext;
    int i,stat;
...
/* search antenna parameter ----------------------------------------------------
*read receiver antenna phase center position
*args   : char   *type       I   antenna type for receiver antenna
*         gtime_t time       I   time to search parameters
*         pcvs_t *pcvs       IO  antenna parameters
*return : antenna parameter (NULL: no antenna)
 *-----------------------------------------------------------------------------*/
extern rpcv_t *searchrpcv(const char *type, gtime_t time, const rpcvs_t *pcvr)
{
    ...
    strcpy(buff,type);
    char *q;
    for (p=strtok_r(buff," ",&q);p&&n<2;p=strtok_r(NULL," ",&q))
        types[n++]=p;
    if (n <=0)
        return NULL;
    ...
    for (i=0;i<pcvr->n;i++) {
        rpcv=pcvr->pcv+i;
        if (strstr(rpcv->type,types[0])!=rpcv->type)
            continue;
...
extern void createdir(const char *path) {
    ...
    strcpy(buff,path);
    if (!(p=strrchr(buff,RTKLIB_FILEPATHSEP)))
        return;
    *p='\0';
