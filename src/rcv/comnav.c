...
static int decode_rangecmpb(raw_t *raw)
{
    ...
    for (i = 0, p += 4; i < nobs; i++, p += 24)
    {
        ...
        if ((sys != SYS_GAL && !clock) || (sys == SYS_GAL && !plock))
            psr = 0.0; /* code unlock */
        if (!plock)
    ...
    if (raw->nbyte == 0)
    {
        for (i = 0;; i++)
        {
            if ((data = fgetc(fp)) == EOF)
                return -2;
            if (sync_cnav(raw->buff, (unsigned char)data))
                break;
            if (i >= 4096)
                return 0;
        }
